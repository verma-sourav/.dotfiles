...
function M.executable(command) return vim.fn.executable(command) == 1 end

function M.merge_tables(base, addl) return vim.tbl_deep_extend("force", {}, base, addl or {}) end
...
function M.glob(pattern) return vim.fn.split(vim.fn.glob(pattern), "\n") end
